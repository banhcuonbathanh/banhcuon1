...
import { FavoriteSetListResType, DishListResType, Dish, FavoriteSet } from '@/zusstand/dished/domain/dish.schema';
...
export function FavoriteSets({ favoriteSets, dishes, onAddToOrder }: FavoriteSetProps) {
  ...
  const createFavoriteSet = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: 'Failed to create favorite set', variant: 'destructive' });
      }
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {favoriteSets.map((set) => (
          <FavoriteSetCard key={set.id} set={set} dishes={dishes} onAddToOrder={onAddToOrder} />
        ))}
      </div>
...
const FavoriteSetCard: React.FC<FavoriteSetCardProps> = ({ set, dishes, onAddToOrder }) => {
  ...
  return (
    <Card className="w-full">
      ...
      <CardFooter>
        <Button onClick={() => setDishes.forEach(onAddToOrder)} className="w-full">
          Add All to Order
        </Button>
      </CardFooter>
    </Card>
