...
const OrderHeader = ({ row, meta }: { row: any; meta: TableMeta }) => {
  ...
  return (
    <div className="flex items-center gap-8 p-4 rounded-t-lg border-b">
      {/* Name Section */}
...
const OrderTracking = ({ row, meta }: { row: any; meta: TableMeta }) => {
  ...
  const totalOrderQuantity = Array.from(dishTotals.values()).reduce(
    (sum, total) => sum + total.quantity,
    0
  );

  const totalDeliveredQuantity = Array.from(deliveryState.values()).reduce(
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-2 px-2 py-1 rounded-t text-sm font-medium">
        ...
        <div className="text-center text-green-600">Delivered Qty</div>
      ...
      {Array.from(dishTotals.entries()).map(([dishName, totals]) => {
        ...
        const remaining = totals.quantity - delivered;
        const isComplete = remaining === 0;
        const remainingAmount = totals.price;
        ...
        return (
          ...
          >
            ...
            <div className="text-center font-medium">{totals.quantity}</div>
      ...
      {/* Totals row */}
      <div className="grid grid-cols-4 gap-2 items-center py-2   font-medium">
        <div className="col-span-1 text-sm">Total</div>
        <div className="text-center">{totalOrderQuantity}</div>
      ...
      return (
        <div className="flex flex-col w-full">
          ...
          <div className="grid grid-cols-3 gap-4 p-4">
            ...
            <div className="space-y-4">
              <h3 className="font-semibold">Bowl Details</h3>
              <BowlDetails row={row} />
            </div>
