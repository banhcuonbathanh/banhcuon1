...
import { useDialogStorePersist } from "../../(landingpage)/landing_page_zustand/landding_page_zustand";
import PassageContend from "./singlereadingfpage_component/passage_component";
import PassageQuestion from "./singlereadingfpage_component/passage_question";
import PassageSummary from "./singlereadingfpage_component/passage_summary";
import Timer from "../../(landingpage)/presentation_component/timer";

interface Params {
  readingid: string;
}
const SingleReadingPage = ({ params }: { params: Params }) => {
  ...
  const openDialog = useDialogStorePersist(
    (state: { openDialog: any }) => state.openDialog
  );
